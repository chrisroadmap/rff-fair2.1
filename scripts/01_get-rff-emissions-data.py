...
co2_hist = (
    df_ssp.loc[
        (df_ssp["Region"] == "World")
        & (df_ssp["Scenario"] == "ssp245")
        & (df_ssp["Variable"] == "Emissions|CO2"),
        "1750":"2020",
    ]
    .interpolate(axis=1)
    .values.squeeze()
...
# would be slightly better to load in default molwts from fair, and much better to use
# fair's inbuilt unit converter
molwt_co2 = 44.009
molwt_c = 12.011
molwt_n2o = 44.013
molwt_n2 = 28.014
...
for sample in tqdm(range(1, 10001), desc="Processing emissions files"):
    ...
    df_out = pd.DataFrame(
        emissions, columns=["CO2", "CH4", "N2O"], index=range(1750, 2301)
    )
